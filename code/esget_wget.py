...
class EsgetWget(object):
    ...
    def mk_wget_file(self, filechunk, host):
        ...
        cert_path = os.path.join(self.wget_remote_dir, 'globus')
        f = open(self.wget_template, "r")
        wget = f.read()
        f.close()
        wgetopts = '" -nv -x -nH -P ' + prefix + ' --no-check-certificate"'
        wget = re.sub('customwget=', 'customwget=' + wgetopts, wget, count=1)
        wget = re.sub('COOKIE_JAR=', 'COOKIE_JAR=' + self.remote_cookies,
                      wget, count=1)
        wget = re.sub('ESG_CERT=', 'ESG_CERT=' +
                      self.remote_certificate, wget, count=1)
...
## copy certificate to zombie
    ...
    def run_esget_getter(self):
        ...
        if False:
            ...
            with open("../tmp/debug_chunks.cpi","r") as f:
                testchunk = cPickle.load(f)[0]
            host = self.hostq.get()
            text = self.mk_wget_file(testchunk, host)
            print(text)
            #self.start_all()
            #self.finish_all()
